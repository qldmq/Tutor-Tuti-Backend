...
@DataJpaTest
public class DisLikeNoticeRepositoryTest {
	@Autowired
	...
	@Autowired
	private DisLikeNoticeRepository dislikeNoticeRepository;
	...
	public void dislikeTest()
	{
		...
		DisLikeNoticeDto disLikeNoticeDto = DisLikeNoticeDto.builder()
											.memberNum(member2Saved.getMemberNum())
											.noticeNum(notice.getNoticeNum())
											.dislikedAt(new Date())
											.build();
		DisLikeNotice dislikeNotice = dislikeNoticeRepository.save(DisLikeNotice.toEntity(disLikeNoticeDto));
		
		assertThat(dislikeNotice).isNotNull();
		assertThat(dislikeNotice.getDislikeNum()).isNotNull();
	}
}
